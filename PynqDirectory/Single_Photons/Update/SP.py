...
PHASE_SHIFTS = 16
FTIME = 1/(PHASE_SHIFTS*REF_CLK)#Shifted Clock Sampling resolution
...
class SP_TOOLS:
    def __init__(self):
        ...
        ##Initialize IDELAY
        self.iDD_DATA = []#AXI GPIO handles for delay data GPIO blocks (which control tap and stages)
    ...
    def st_arm_and_wait(self):
        ...
        dels = self.ST_DATA.read(ch2_data)#Read start and finish delay line values
        ...
        #to the coarse time
        op = op + (((dels&0b111111110)>>1)-((dels&0b11111111000000000)>>9))*FTIME
    ...
    def ct_arm_and_wait(self,first):
        ...
        dels = self.CT_DATA.read(ch2_data)#Read delay line states including the data ready bit which must be shifted out below
        #Include fine time offsets with the coarse time
        tm =tm+ (((dels&0b111111110)>>1)-((dels&0b11111111000000000)>>9))*FTIME
        self.CT_UTIL.write(ch1_data,0x0)#Disable coincidence timer by placing it in reset
        return tm
    ####---------------------Signal generator---------------------------------------------####
    ...
    def TT_proc(self):
        ...
        dels = self.TT_DELAY_DATA.read(ch1_data)#Delay line states for channels 0-3 (Each is concatenated in binary)
        ...
        ctime1 = cttime1/REF_CLK - stimet1 + stimet0
        ...
        timeouts = (self.TT_UTIL.read(ch2_data)&0b11110) >> 1#Read time outs and shift out the data ready line
    ...
    def DD_idelay(self,channel,tap,stage):
        ...
        """
        plog.info("Setting input delay on channel "+str(channel)+" dline tap of "+ str(tap)+" with "+str(stage) +" stage(s).")
        self.iDD_UTIL[channel].write(ch1_data,0x0)#Reset and unreset the delay block
        sleep(slt)
        self.iDD_UTIL[channel].write(ch1_data,0x1)
        self.iDD_DATA[channel].write(ch1_data,tap)#Load the delay line tap for the top stage
        self.iDD_DATA[channel].write(ch2_data,stage)#Select how many stages are activated below the top stage
