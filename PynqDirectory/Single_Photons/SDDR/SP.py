...
class SP_TOOLS:
    ...
    def st_arm_and_wait(self):
        ...
        while(self.ST_DATA.read(ch2_data))==0:#Wait for ready
            pass
    ...
    def ct_arm_and_wait(self,mode):
        ...
        if(sel == LineSelectMode.DONTCARE):
            self.CT_UTIL.write(0x0,self.UTIL.read(0x0)|0b100)
        else:
            self.CT_UTIL.write(0x0, self.UTIL.read(0x0) & 0b001)
            self.CT_UTIL.write(0x0,self.UTIL.read(0x0) | (mode << 1))
        self.CT_UTIL.write(0x0,self.UTIL.read(0x0) | 0b1)
        #Wait for the coincidence time to be ready (waits until the second pulse)
        while(self.CT_UTIL.read(ch2_data))==0:
            pass
        #Read coarse time
        tm = self.CT_DATA.read(ch1_data)/DET_REF_CLK
        dels = self.CT_DATA.read(ch2_data)#Read fine times
        #Include fine time offsets with the coarse time
        tm =tm+ ((dels&0xFF)-(dels&0xFF00)>>8)*FTIME
